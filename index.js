...
function cumlelereDonustur(cumleler, ayrac=",") {
  let YeniCumleler=[];
  YeniCumleler=cumleler.map(cumle=> {return cumle.join(ayrac);} 
    );
  return YeniCumleler;
}
...
function paragrafOlustur(cumleler,cumleKur,cumlelereDonustur) {
  let ilkBesCumle=[];
  let YeniCumlelerDizi=[];
  YeniCumlelerDizi=cumlelereDonustur(cumleler," ")
  for (i=1;i<=9;i=i+2){
    ilkBesCumle.push(YeniCumlelerDizi[i]);
  }
  return ilkBesCumle;
}
...
unction emojileriDonustur(cumle,emojiler) {
